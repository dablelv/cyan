...
func TestBase64TripleDESCBCEncrypt(t *testing.T) {
	p := []byte("plaintext")
	key := []byte("12345678abcdefgh12345678")
	c, _ := Base64TripleDESCBCEncrypt(p, key)
	assert.Equal(t, "dau0DzmDGQbHasZaOvxxwg==", c)
}

func TestBase64TripleDESCBCDecrypt(t *testing.T) {
	c := "dau0DzmDGQbHasZaOvxxwg=="
	key := []byte("12345678abcdefgh12345678")
	p, _ := Base64TripleDESCBCDecrypt(c, key)
	assert.Equal(t, "plaintext", string(p))
}
